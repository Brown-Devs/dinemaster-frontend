...
export default function MasterProductFormModal({ open, onClose, product = null }) {
  const { masterCategoriesQuery, createMasterProductMutation, updateMasterProductMutation } = useMasterProducts();
  ...
  const uploadImage = async (file) => {
    ...
    fd.append("file", file);
    fd.append("companyId", "master"); // Fixed ID for Master Catalog
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (product?._id) {
        await updateMasterProductMutation.mutateAsync({ id: product._id, data: payload });
      } else {
        await createMasterProductMutation.mutateAsync(payload);
      }
      onClose();
    } catch (err) {}
  };

  const isSubmitting = createMasterProductMutation.isPending || updateMasterProductMutation.isPending || isUploading;
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-border">
        <div>
          <p className="font-bold text-lg" style={{ color: "var(--fg)" }}>
            {product ? "Edit Master Product" : "Add New Master Product"}
          </p>
          <p className="text-xs" style={{ color: "var(--muted)" }}>
            {product ? "Update master catalog details" : "Create a new entry for the global catalog"}
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
            ...
            <div className="flex flex-col sm:flex-row gap-5">
              ...
              <div className="sm:w-40 shrink-0">
                ...
                <div
                  ...
                  onClick={() => document.getElementById("master-product-img-input").click()}
                  ...
                  {previewUrl ? (
                    ...
                    <div className="flex flex-col items-center gap-2 p-4 text-center">
                      ...
                      <span className="text-[11px]" style={{ color: "var(--muted)" }}>
                        Drop or click<br />
                        <span className="font-semibold">Max 800KB</span>
                      </span>
                    </div>
                  )}
                </div>
                ...
                <input id="master-product-img-input" type="file" hidden accept="image/*" onChange={handleFileChange} />
              </div>
        ...
        <div className="flex items-center justify-end gap-3 px-5 py-4 border-t border-border">
          ...
          >
            {isUploading
              ? "Uploading..."
              : product ? "Update Master Product" : "Create Master Product"}
          </Button>
        </div>
      </form>
    </Dialog>
